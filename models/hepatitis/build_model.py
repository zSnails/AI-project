...
def split_data() -> None:
    ...
    DataFrame({"Category": data["Category"], "Category_Code": data["Category_Code"]}).to_csv("category_and_category_codes.csv")
    DataFrame({"Sex": data["Sex"], "Sex_Code": data["Sex_Code"]}).to_csv("sex_and_sex_codes.csv")

    train_data: DataFrame
    test_data: DataFrame
    train_data, test_data = train_test_split(data, test_size=.3, random_state=67) # type:ignore
    train_data.to_csv("train_data.csv")
    test_data.to_csv("test_data.csv")

def test_model() -> None:
    model: RandomForestRegressor = load("./hepatitis.model.pkl")
    test = read_csv("./test_data.csv")
    x_test = test[["Age", "Sex_Code", "ALB", "ALP", "ALT", "AST", "BIL", "CHE", "CHOL", "CREA", "GGT", "PROT"]]
...
# R² = 0.9123614589785785                                                                                                                                                                       
# RMSE = 0.2929993867759935
